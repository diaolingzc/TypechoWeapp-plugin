...
$users = $db->fetchAll($db->select()->from('table.weapp')->order('table.weapp.create_at', Typecho_Db::SORT_DESC));

$pageCount = ceil( count($users)/$pageSize );

$userpage = $db->fetchAll($db->select()->from('table.weapp')
    ->page($currentPage, $pageSize)
    ->order('table.weapp.create_at', Typecho_Db::SORT_DESC));
        ...
        <div class="row typecho-page-main" role="main">
            <div class="col-mb-12 typecho-list">
                <div class="typecho-list-operate clearfix">
                    <form method="POST" action="<?php $options->adminUrl('extending.php?panel=TypechoWeapp%2FWeUsers.php'); ?>">
                            ...
                            <tbody>
                                <?php foreach($userpage as $user): ?>
                                    <tr>
                                        ...
                                        <td><?php echo date('Y-m-d H:i:s', $user['create_at']); ?></td>
                                        <td><?php echo date('Y-m-d H:i:s', $user['lastlogin_at']); ?></td>
